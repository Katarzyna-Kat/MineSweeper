...
class MyApp(App):
    ...
    def main(self):
        ...
        self.horizontal_container.append(
            [imgFlag, self.lblFlagCount, self.lblTime, self.btReset]
        )
    ...
    def new_game(self, widget):
        ...
        for x in range(0, len(self.mine_matrix[0])):
            ...
            for y in range(0, len(self.mine_matrix)):
                ...
                self.mine_matrix[y][x].onclick.connect(
                    self.mine_matrix[y][x].check_mine
                )
    ...
    def check_if_win(self):
        ...
        if win:
            self.dialog = gui.GenericDialog(
                title="You Win!", message="Game done in %s seconds" % self.time_count
            )
                ...
                if (not self.mine_matrix[cell.y][cell.x].has_mine) and (
                    ...
                    for coord in [
                        ...
                        if not self.mine_matrix[cell.y + _y][cell.x + _x].opened:
                            self.mine_matrix[cell.y + _y][cell.x + _x].check_mine(
                                None, False
                            )
